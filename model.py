...
tf.compat.v1.disable_eager_execution() # In order to prevent the error caused by version incompatibility.
...
class CustomLayer(keras.layers.Layer):
    ...
    def combined_loss(self, x, z_decoded):
        x = K.flatten(x)
        ...
        # KL Divergence Loss
        kl_loss = -0.01 * 0.5 * K.mean(1 + z_sigma - K.square(z_mu) - K.exp(z_sigma), axis=-1)  # Beta = 0.001
    ...
    def call(self, inputs):
        x = inputs[0]
        z_decoded = inputs[1]
        loss = self.combined_loss(x, z_decoded)
        self.add_loss(loss, inputs=inputs)
        return x

# Applying Custom Loss

y = CustomLayer()([input_image, z_decoded])
